...
import {
  userConnected,
  userNotConnected,
} from "./redux/actions/internetActions/internetActions.ts";
...
export const App = () => {
  ...
  const cart: any = cartState.cart;

  const bookState: any = useSelector((state: RootState) => state.Books);
  const bookStateHasChanged: any = bookState.bookStateHasChanged;
  ...
  }, [userLoggedIn, bookStateHasChanged]);
